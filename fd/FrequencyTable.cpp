...
void reader(const commandParameters& parameters,size_t startPos, size_t intervalLength,FrequencyTable* subResult,int threadIndex)
{
    ...
    std::ifstream ifs ;
    ifs.open(parameters.file,std::ios::in);
    ifs.seekg(startPos,std::ios::beg);

    char buffer = '\0';
    unsigned char uBuffer = '\0';
    for (uint32_t i=0;i<intervalLength;++i) //i++, ++i ?
    {
        ifs.get(buffer);
        uBuffer = buffer;
        ++temporary[uBuffer];
...
FrequencyTable buildFrequencyTable(const commandParameters& parameters)
{
    ...
    {
        threadReader[i] = std::thread(reader,parameters,average*i,average,subFrequencyTable+i,i);
    }
    ...
    threadReader[threadCount-1] = std::thread(reader,parameters,average*(threadCount-1),ending,subFrequencyTable+threadCount-1,threadCount-1);
